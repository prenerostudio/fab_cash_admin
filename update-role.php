					...
					<div class="col-xl-12 col-lg-12">
						<div class="card">
							<div class="card-header">
								<h4 class="card-title">Update User Status</h4> </div>
							<div class="card-body">
								<div class="basic-form ">
									...
									<div class="form-group">
										...
										<div class="form-group">
										<label class="mb-1"><strong>User Mobile</strong></label>
...
?>
   ...
   $fetchQuery = mysqli_query($connection, "SELECT * FROM `packages` WHERE `package_id` = '$u_status'");
$pRow = mysqli_fetch_array($fetchQuery);
$packageName= $pRow['package_name'];
$pacakgeDays = $pRow['package_duration'];


// $query1="UPDATE user_register SET u_fname = '$u_fname', package_id = '$u_status', u_phone= '$u_phone' WHERE user_id = '$id'";
   $query1 = "INSERT INTO `active_packages`(`u_id`, `package_id`, `package_name`, `package_days`, `active_date`) VALUES ('$id','$u_status','$packageName', '$pacakgeDays', '$date')";
  $exe1 = mysqli_query($connection,$query1);
  if ($exe1) {
	$querydlt = "DELETE FROM transition_img WHERE user_phone = '$u_phone'";
 $exedlt = mysqli_query($connection,$querydlt);
if ($exedlt) {
	?>
    <script>
   alert('Status Updated');
        setTimeout(function(){location.href="viewuserinfo.php"} , 2000);
  </script>    
  <?php
  }
  }else{
